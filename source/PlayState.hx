...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.stage)
		{
			case 'morning':
			{
				curStage = 'nevada';
				defaultCamZoom = 0.75;
				var city:FlxSprite = new FlxSprite(-600, -400).loadGraphic(Paths.image('accelerant/city'));
				add(city);
				heli = new FlxSprite(-1000, -50);
				heli.frames = Paths.getSparrowAtlas('accelerant/heli');
				heli.animation.addByPrefix('heli', 'heli');
				add(heli);
				var cliffs:FlxSprite = new FlxSprite(city.x, city.y).loadGraphic(Paths.image('accelerant/CLIFFS'));
				add(cliffs);
				var floor:FlxSprite = new FlxSprite(city.x, city.y).loadGraphic(Paths.image('accelerant/floor'));
				add(floor);
				gf = new Character(0,0,'speaker');
				add(gf);
				remove(gf);
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Fucked" : storyDifficulty == 1 ? "Hard" : "Baby") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (SONG.noteStyle)
			{
				...
				default:
					babyArrow.frames = Paths.getSparrowAtlas('STRUMLINE_assets');
					babyArrow.animation.addByPrefix('green', 'arrowUP');
					babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
					babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
					babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (!trickster)
						{
							...
							{
								case 2:
									tricky.playAnim('singUP' + altAnim, true);
								case 3:
									tricky.playAnim('singRIGHT' + altAnim, true);
								case 1:
									tricky.playAnim('singDOWN' + altAnim, true);
								case 0:
									if (curStep != 735)
										tricky.playAnim('singLEFT' + altAnim, true);
							}
						}
					...
					if (daNote.mustPress && !daNote.modifiedByLua)
					{
						daNote.visible = playerStrums.members[Math.floor(Math.abs(daNote.noteData))].visible;
							daNote.x = playerStrums.members[Math.floor(Math.abs(daNote.noteData))].x - (daNote.noteType == 2 ? 25 : 0);
						if (!daNote.isSustainNote)
					...
					else if (!daNote.wasGoodHit && !daNote.modifiedByLua)
					{
						...
						daNote.x = strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].x - (daNote.noteType == 2 ? 25 : 0);
						if (!daNote.isSustainNote)
					...
					if (daNote.tooLate || daNote.wasGoodHit)
					{
						...
						{
							dad.playAnim('sing' + directions[daNote.noteData] + 'shoot');
						}
	...
	*/
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'accelerant')
		{
			switch (curStep) {
				case 670:
					remove(gf);
					gf = new Character(gf.x, gf.y + 350, 'bf-speaker');
					add(gf);
				case 671:
					tricky.setPosition(gf.x, gf.y - 500);
					tricky.visible = true;
				case 672|864:
					trickster = true;
				case 735:
					tricky.playAnim('hank', true);
				case 736 | 928:
					trickster = false;
				case 936:
					dad.playAnim('shootTrickster');
					remove(gf);
					tricky.playAnim('fall');
					add(gf);
					FlxTween.tween(tricky, {y: 1000}, 0.5);
	...
	{
		super.beatHit();
		if (curBeat % 3 == 0 && curStage == 'nevada') {
			if (FlxG.random.bool(25)){
				heli.animation.play('heli', true);
				heli.x = -10000;
				heli.y = -100;
				FlxTween.tween(heli, {x: 10000}, 10);
			}
		}
